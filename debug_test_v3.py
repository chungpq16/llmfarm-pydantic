...
class CustomAsyncClient(httpx.AsyncClient):
    """Custom HTTP client that handles the API version parameter correctly"""
    ...
    async def request(self, method, url, **kwargs):
        # If the URL ends with /chat/completions, add the api-version parameter
        if str(url).endswith('/chat/completions'):
            # Add api-version query parameter
            if '?' not in str(url):
                url = f"{url}?api-version=2024-08-01-preview"
            else:
                url = f"{url}&api-version=2024-08-01-preview"
...
class LLMFarmPydantic:
    def __init__(self, model="gpt-4o-mini"):
        ...
        # Base URL without /chat/completions and without api-version
        # The OpenAI client will add /chat/completions, and our custom client will add api-version
        ...
        # Create custom HTTP client that will handle the api-version parameter
        try:
            custom_http_client = CustomAsyncClient(
                headers=headers,
                timeout=30.0
            )
...
if __name__ == "__main__":
    print("=== LLM Farm PydanticAI Debug Test (Custom HTTP Client Version) ===")
